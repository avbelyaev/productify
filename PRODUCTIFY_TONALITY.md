...
Не ничего проще `echo`. Давай попробуем помесить `echo fuck` в образ? 
Ведь процесс`echo fuck` ничем не отличается от `python3.8 my-awesome-script.py`.

1. Важно помнить следующее: **контйенер жив, пока в нем живет процесс**. Как только процесс внутри контейнера останавливается, 
считается, что дальше крутить контейнер нет смысла. То есть если просто вывести 1 раз echo, то ровно это и прозойдет - 
контейнер создастся, echo напишет fuck, зваершится и контйенер будет остановлен.

2. А еще важно помнить, что у Bash'а есть альтернативная форма записи компанд:
```bash
bash -c "echo fuck"
# это то же самое, что и
echo fuck
```
Здесь флагом `-c` обозначается команда.

Принимая пункт 1. во внимание, можем сделать бесконечный цикл, который бы писал факи:
```bash
...
```bash
bash -c "while true; do echo fuck; sleep 2; done"
```

Итак, цикл бесконченый, значит процесс и контейнер не умрут. Можем собирать


Создадим следующий файл `Dockerfile` (у докерфайла нет расширения):
```
FROM ubuntu:18.04
...
В качестве процесса можно запустить цикл, который будет бесконечно ругаться:
```bash
while true; do echo fuck; sleep 2; done
```
Попробуй для начала выполнить эту компанду в терминале. 
...
хранятся в docker registry, реестре докер образов. Самый известный - Dockerhub - публичное хранилище докер образов.
Еще есть твое локальное хранилище у тебя на компе (`docker images`). Компании, как правило, имеют свои приватные хранилища.

Хотя сам докер и проприетарен (закрытое ПО), механизм создания образов открыт доступен всем. Благодаря этому, любой
может создать образ. Представь, что 1000 человек взяли за основу образ python, добавили сверху исходники своих программ 
...
поверх питоном. Тогда хранить надо лишь 1! образ linux, 1! образ питона и 1000 папочек с исходниками.
...
a4d0e23f5eaf: Extracting [=========>                                     ]  13.37MB/71.42MB
...
* Разумеется, слоев "между тобой и базовым" не 4 (scratch, ubuntu, python, твой), а скорее десяток, но сути это не меняет. 


### Версионирование

Как ты знаешь, образ характериузется версией. Это называется тегом, например, `python:3.8` - здесь `3.8` - тег.
Тэг `latest` - плавающий - он всегда указывает на самую последнюю новую версию образа.
...
исходников. Это считается дурной практикой и как правило запрещено реестром. Как описано выше, образы очень экономно 
расходуют место и теги можно не жалеть. 

Так что "в продакшене" под одним тегом всегда должен быть только один образ. Иммутабельность наше все :)


### Как выбрать образ

Обычной практикой является гугление `<whatever> docker image` и, если это что-то достоано мейнстримное, 
первая ссылка будет вести на Dockerhub. Но, в случае с чем-то супер-мейнстримным, типа питона, ты столкнешься с тем,
что образом сотни. Если глянуть на вкладку Tags у питона, там будут как минимум 
[52 страницы](https://hub.docker.com/_/python?tab=tags&page=52) образов (нояб 2020). 

В какой-то момент умные люди перестали это терпеть и выделили т.н. alpine-образы. 

Ты много где увидишь/услышишь слово "alpine" в контексте докера. Alpine - это легковесный дистрибутив линукса.
В то время, как Ubuntu, Fedora, Mint, Arch и т.д.весят сотни МБ или даже пару ГБ, Alpine, идейно являясь полноценным 
linux'ом не тащит за собой мусор - ему не нужны драйвера для мышки и набор иконок - это все вырезано. 
Поэтому, он весит всего 5 мегабайт! 

Вслед за alpine-linux появились и облегченные версии всех возможных технологий. "Обычный" питон может весить до 500МБ,
alpine-python весит 100МБ, ведь из него вырезали лишние предустановленные библиотеки, профилировщик, справки и тд. 
...
И да, `alpine-python-3.8`, `python-3.8-alpine`, `3.8alpine` и `alpine3.8` - это все еще __разные__ образы! Идея с алпайнами
не сильно упростила процесс выбора версии. Раньше ты выбирала из океана образов нужный тебе образ, сейчас
ты из моря легковесных образов выбираешь подходящий.

Иногда alpine может выстрлить в ногу, когда в нем не окажется нужной твоему модному пайспарку библиотеки, 
написанной сербом в 1998 году. Я на практике такого не встречал, но это стоит иметь в виду.

А вот nano, vim, netstat, curl и даже bash в алпайне может не быть. Это все пользовательские отладочные утилиты и в
