...
- мой браузер делает запрос на хост 1.2.3.4 и порт :80 за стартовой страничкой index.html
    ...
    - я ввожу текст и нажимаю кнопку "Analyze". Внимание, вопрос, куда уйдет запрос? 
        - На какой хост:порт? 
        - кто его на этом хосту:порту примет?
        - ведь тот питоновский веб-сервер умеет только отдавать статику (html/css/js - "статический" контент, т.к. почти никогда не меняется)
        - а Flask запушен отдельным процессом и вот имеено он занимается анализом текста, т.е. бизнес-логикой и нам хочется достучаться до него
