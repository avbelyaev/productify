...
3. SQLite (база данных в виде одного файлика (а может и вообще без ничего работать - напрямую из памяти), крайне простая, но из-за того оч ограниченная поддержка SQL. зато ее удобно передавать кому-нибудь, т.к. вся база в одном файлике)
...
5. Oracle (нерельно запустить локально, платная, быстрая, адски-страшная, тот самый "кровавый энтерпрайз")
6. csv, txt файлик
7. MS Excel + Access
...
- запросы никто не оптимизирует и они реально тупые и топорные.
    - но ты уже используешь ORM и на чистом SQL тебе больше никто писать не даст, иначе вся магия не сработает. Тогда такой запрос:
```sql
...
```
придется переписыватть вот так:
```python
...
2.Такой способ применяется в джаве - он более наивный. Джавовый ORM (называется Hibernate) читает файлик Film.java. Натурально, читает как строку. 
Затем парсит его и ищет те же самые название класса, названия полей, и генерит такой же SQL-запрос. Да, да, настолкьо тупо. Все из-за того, что `private` в Java
это реально приватно и получить информацию в рантайме оттуда нельзя. В старых версиях джавы единственно возможным способом было тупо распарсить текстовый файл класса.

Сейчас все немного иначе - инфа вытягивается из "собачек":
```java
@Entity(table = "films")
public class Film {
...
- Вместо того, чтобы делать `UPDATE films set year_of_release=2020 ...`, он делает то же самое, но в памяти:
    - берет объект, который отвечает за ту строку и меняет дату релиза в нем на 2020.
- Вот мы захотели изменить и название.
    - берем тот же объект и меняем в нем еще и название
- но ту хобана! мы захотели достать все фильмы, выпущенные в 2020
    - но как быть? ведь в базе-то по-прежнему стоит 2016 - мы же туда ни разу не дампили изменения
    - и вот тут ORM замечает, что этот запрос пересекается с тем, что находится у нас в памяти
        ...
        - вычисляет diff (разницу) между такими объектами
        - генерирует SQL-запрос типа `UPDATE films set year=2020, name='mr strange' WHERE ...`
             - этот запрос приводит объект в базе в ожидаемое состояние
        - выполняет UPDATE-запрос
    - теперь в базе консистетные свежие данные
    - теперь ORM выполяет тот запрос, который нам был нужен
    
    
Как ты понимаешь, весь это процесс нетривиален. Еще нетривиальнее он становится, когда у нас 100500 потоков и все хотят в базу и все параллельно что-то обновляют. И он становится капец медленным и неоптимальным.

Как итог, ORM становится бутылочным горлышком, и по своей природе не может никого другого пустить в базу. Вот почему, используя ORM ты больше не сожмешь писать на SQL в жтом же приложении - ведь иначе твой SQL пройдет мимо кассы и ORM ошибется (например, при вычислении diff'а).
